...
for row in rows:
    ...
    try:
        cursor.execute(f'''UPDATE {current_db} SET DT = {round(downtime, 2)}, MTTR = {round(downtime_avg, 2)}, MTBR = {round(time_between_avg, 2)}, COUNT = {count} WHERE EQUIPMENT = {machine} ''')
    except:
